...
function renderStack(){
  ...
  if (ALL_STACKS.length===1){
    selected.stack=ALL_STACKS[0];
    els.stackGroup.style.display="none";
    return;
  }
...
function renderSize(){
  ...
  if (SEQ_MODES.includes(selected.mode)&&selected.villain&&selected.hero&&selected.stack){
    ...
    const THREEBET_OPEN_SIZES = APP_MODE === "noante" ? [2.5] : ALL_OPEN_SIZES;
...
function resetAll(){
  selected={mode:"raise",stack:100,hero:"BB",villain:"UTG",villain2:null,openSize:2.5,threebetSize:null,betSize:null,limpSeq:null};
  syncHash(); refreshAll();
}
...
async function init(){
  try{
    ...
    selected={...selected,mode:"raise",hero:"BB",villain:"UTG",villain2:null,openSize:2.5,threebetSize:null,betSize:null,limpSeq:null};
