...
def parse_chart(base, stack, folder, folder_open, file_path):
    ...
    # --- limp-pot tobbutas: strukturalt kategoriak + rovid seq-cimkek ---
    # vsopenlimp: 0 raise (valasz a limpre) | vsiso: 1 raise, hero != limper
    # faceiso: 1 raise, hero == limper      | limp3b: 2 raise (3bet a limp-potban)
    if folder == 'limp':
        ...
        if first and first[1] == 'C':
            ...
            def _slab(s):
                if s[1] == 'R':
                    return s[0].upper() + ' ' + ('%g' % s[2]) + 'bb'
                return s[0].upper() + ' call'
            ...
            if nr == 2:
                ...
                if hero != rsteps[0][0]: return None
        ...
        if firstp and hero == firstp:
            return E(facing='faceiso', villain=(raisers[-1] if raisers else None), seq_key=base)
        return E(facing='vsopenlimp', villain=firstp, seq_key=base)
